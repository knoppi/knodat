...
def plotg(dataFileName, **opts):
    ...
    for opt,val in opts.items():
        if opt == "-x" or opt == "--xcol":
            x_col = val
        if opt == "-y" or opt == "--ycol":
            y_col = val
        if opt == "-c":
            z_col = val
        if opt == "-N":
            N = int(val)
        if opt == "--incomplete" or opt == "-i":
...
def usage():
    ...
    -x, --xcol NUMBER           use the given column for the x-values (defaults to 1)
    -y, --ycol NUMBER           use the given column for the y-values (defaults to 2)
